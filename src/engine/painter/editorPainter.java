        ...
        try {
            ...
            for (Boat boat : game.getHumanPlayer().getBoard().getBoats()) {
                ...
                if (boat.getOrientation() == Orientation.VERTICAL) {
                    ...
                    im.getGraphics().drawImage(boat1img,  Constant.CASE_WIDTH * Constant.WIDTH, boat.getPosition().getY() * Constant.CASE_HEIGHT + 2*Constant.CASE_HEIGHT, boat.getSize() * Constant.CASE_WIDTH,  Constant.CASE_HEIGHT, null);
            ...
            for (Position pos : game.getHumanPlayer().getBoard().getShoots().keySet()) {
                ...
                if (game.getHumanPlayer().getBoard().getShoots().get(pos) == null)
                    im.getGraphics().drawImage(image1 , (pos.getX() )* Constant.CASE_WIDTH , pos.getY()* Constant.CASE_HEIGHT + Constant.CASE_HEIGHT * 3 , Constant.CASE_WIDTH , Constant.CASE_HEIGHT, null);
                else
                    im.getGraphics().drawImage(image2 , (pos.getX())* Constant.CASE_WIDTH , pos.getY()* Constant.CASE_HEIGHT , Constant.CASE_WIDTH , Constant.CASE_HEIGHT, null);
